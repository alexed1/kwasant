...
namespace KwasantTest.Managers
{
    ...
    public class CalendarSyncManagerTest
    {
        ...
        public async void CanAddToLocalCalendar()
        {
            ...
            Assert.AreEqual(0, _uow.EventRepository.GetAll().Count(e => e.StateID != EventState.Deleted), "No events must be in the repository before synchronization.");
            await _calendarSyncManager.SyncNowAsync(_uow, _curUser);
            _uow.SaveChanges();

            // VERIFY
            var events = _uow.EventRepository.GetAll().Where(e => e.StateID != EventState.Deleted).ToArray();
        ...
        public async void CanAddToRemoteCalendar()
        {
            ...
            // EXECUTE
            Assert.AreEqual(1, _uow.EventRepository.GetAll().Count(e => e.StateID != EventState.Deleted), "One event must be in the repository before synchronization.");
        ...
        public async void CanRemoveFromLocalCalendar()
        {
            ...
            // VERIFY
            Assert.AreEqual(0, _uow.EventRepository.GetAll().Count(e => e.StateID != EventState.Deleted), "No events must be in the repository after synchronization.");
        ...
        public async void CanMergeCalendars()
        {
            ...
            // VERIFY
            var localEvents = _uow.EventRepository.GetAll().Where(e => e.StateID != EventState.Deleted).ToArray();
