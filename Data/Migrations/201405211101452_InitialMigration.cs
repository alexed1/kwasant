...
namespace Data.Migrations
{
    ...
    public partial class InitialMigration : DbMigration
    {
        ...
        {
            Sql(@"/* Drop all non-system stored procs */
...
END");
            Sql(@"/* Drop all Foreign Key constraints */
DECLARE @name VARCHAR(128)
...
SELECT @name = (SELECT TOP 1 TABLE_NAME FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE constraint_catalog=DB_NAME() AND CONSTRAINT_TYPE = 'FOREIGN KEY' ORDER BY TABLE_NAME)
END");
            Sql(@"/* Drop all Primary Key constraints */
DECLARE @name VARCHAR(128)
DECLARE @constraint VARCHAR(254)
DECLARE @SQL VARCHAR(254)
...
SELECT @name = (SELECT TOP 1 [name] FROM sysobjects WHERE [type] = 'U' AND Name != '__MigrationHistory' AND category = 0 ORDER BY [name])

WHILE @name IS NOT NULL
BEGIN
	IF @name = '__MigrationHistory'
		CONTINUE
